...
function gradientBackgrounds_update(xMousePos, yMousePos) {
	...
	gradientBackgrounds.forEach(background => {
		// Get the x and y mouse position relative to the background's containing <section> represented as a negative number
		const section = background.closest('section');
		let xPos = (xMousePos - section.getBoundingClientRect().left) / (section.offsetWidth) / 2 * 100 * -1;
		let yPos = (yMousePos - section.getBoundingClientRect().top) / (section.offsetHeight) / 2 * 100 * -1;

		// Limit the number to between -50% and -0%
		if (xPos < -50) {
			xPos = -50;
		}
		else if (xPos > 0) {
		...
		// Apply xPos and yPos to the opposite axis, as this looks cooler :)
		background.style.left = yPos+'%';
		background.style.top = xPos+'%';
